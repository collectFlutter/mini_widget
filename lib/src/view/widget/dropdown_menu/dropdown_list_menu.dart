...
typedef MenuTreeOnTap<T, E> = void Function(
    T? data, int? index, E? subData, int? subIndex);
...
class CheckItem<T> {
  ...
  CheckItem({this.check = false, required this.t});
}
...
class _TreeMenuList extends DropdownState<DropdownTreeMenu> {
  ...
  Widget buildItem(BuildContext context, int index) {
    final dynamic list = widget.data;
    final dynamic data = list[index];
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
              ...
              child: CustomScrollView(
                slivers: <Widget>[
                  SliverList(
                    ...
                    buildSubItem,
                    childCount: _subData == null ? 0 : _subData.length,
                  ))
                ],
              ),
            ))
